...
(function (window) {
    define([
        ...
        var DownloadListView = React.createClass({displayName: 'DownloadListView',
            ...
            render : function () {
                ...
                return (
                    React.DOM.div( {className:"o-download-list-container"}, 
                        ...
                        React.DOM.ul( {className:this.state.containerClass}, 
                            this.createList(videoEpisodes)
                        ),
                        this.state.displayShowAll && React.DOM.a( {onClick:this.showAll, className:"w-text-secondary"}, FormatString(textEnum.SHOW_ALL, [videoEpisodes.length + 1]))
                    )
                );
            },
            showAll : function () {
